McCOIL_categorical = function(data, maxCOI=25, threshold_ind=20, threshold_site=20, totalrun=10000, burnin=1000, M0=15, e1=0.05, e2=0.05, err_method=1,
		              path="/home/tfarrell/coil/THEREALMCCOIL/categorical_method", output="output.txt", P=c(), filter_sites=TRUE, write_files=0) {
	
	## To allow for specifying minor allele frequencies (MAFs) for SNP data with small number of samples,  
	## add a default parameter P equal to empty vector. Then, before computing MAFs (P), check if  
	...
	mcCoil_categorical_code_location = '/McCOIL_categorical_code_linux.so'
	...
	#print(In_ind)
	#print(In_site)
	## if P has not been pre-specified, compute minor allele frequencies (P) at each site
	if (length(P) == 0) {  
	...
	if ((n>10 & k>10)){	
		...
		K <- .C("McCOIL_categorical", as.integer(maxCOI), as.integer(totalrun), as.integer(n), as.integer(k), 
		     			      as.double(simpleS2_vec), as.integer(M0), as.double(P0), as.double(e1), 
					      as.double(e2), as.character(output), as.character(path), as.integer(err_method), as.integer(write_files))
	   ...
	   if (err_method == 3) {
	    ...
	    if ((err_method == 1) | (err_method == 2)) {
	       output_sum= data.frame(cbind(rep(output, (n+k)),	c(rep("C", n), rep("P", k)), 
				      	    c(select_ind, select_pos), c(meanM, meanP), c(medianM, medianP), 
					    round(c(sdM, sdP),digits=5), c(M025, P025), c(M975, P975)))
	    } else {
		output_sum= data.frame(cbind(rep(output, (n+k+2)), c(rep("C", n), rep("P", k), "e1", "e2"), 
			    		     c(select_ind, select_pos, "e1","e2"), c(meanM, meanP, mean_e1, mean_e2), 
