...
class MAF:
	...
	def filter_missing_positions(self, barcode_seq): 
		return(filter(lambda (pos, base): pos in self.positions(), enumerate(list(barcode_seq))))
	...
	def getProbability(self, position, coi):
		return self.position_to_maf_map()[position[1]].getCallProbability(position[0], coi)
	
	def addPosition(self, MAFPos):
		self.mafs.append(MAFPos)
		
	def position_to_maf_map(self):
		pos2maf = {}
		for maf in self.mafs: pos2maf[maf.pos] = maf
		return(pos2maf)
...
class MAFPosition:
	...
	def getMAFFromChars(self):
		all_chars = set(self.chars)
                #~ if len(all_chars) > 6:  ##this is when rough COI is used in calculation 
                if len(all_chars) > 3:
