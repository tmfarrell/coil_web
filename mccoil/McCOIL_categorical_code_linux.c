...
void McCOIL_categorical(int *max, int *iterations, int *n0, int *k0, double *sampleS2, int *M0, double *P0, double *error1, double *error2, char **file_index, char **path, int *err_method0, int *write_files) {
...
///////don't change////////
	...
	int max_moi= *max;	
	int iter = *iterations;
	int write = *write_files; 
	int n = *n0, k = *k0;
	//for err_method: 
        //1: use pre-specified e1 and e2; 
        //2: use likelihood-free sampling for e1 and e2; 
	//3: update e1 and e2 according to likelihood (for 2 and 3, pre-specified e1 and e2 were used as initial values) 
	...
	if (write != 0) V0 = fopen(var_file, "w");
	...
	// transfer final vectors to function args, 
	// so can be accessed by result of .C() in McCOIL_categorical R function
	for (i = 1; i <= n; i++){
	  M0[i-1]= M[i];
	} 
	for (j = 1; j <= k; j++) { 
